...
def get_frame_for_not(video_path,index):
    cap = cv2.VideoCapture(video_path)
    ...
    if len(variables) < 6:
        print(f"Error: {file_name} has too few parts ({len(variables)}). Skipping...")
        return
    ...
    try:
        ...
        v_start_sec = int(v_start_time[:2]) * 60 + int(v_start_time[2:])
        v_end_sec = int(v_end_time[:2]) * 60 + int(v_end_time[2:])
        ote_start_sec = int(ote_start_time[:2]) * 60 + int(ote_start_time[2:])
        ote_end_sec = int(ote_end_time[:2]) * 60 + int(ote_end_time[2:])
    except Exception as e:
        print(f"Error parsing time for {file_name}: {e}")
        return
    
    segment_type = index % 3

    if segment_type == 0:
        start_time = 1
        end_time = v_start_sec
        region_name = "before_v"

    elif segment_type == 1:
        start_time = v_end_sec
        end_time = ote_start_sec
        region_name = "between_v_ote"

    else:
        start_time = ote_end_sec
        end_time = (total_frames / fps)-0.5
        region_name = "after_ote"

    if end_time <= start_time:
        print(f"Skipped {file_name}: invalid interval ({region_name}) {start_time}–{end_time}")
        return
    ...
    if start_frame >= total_frames or end_frame >= total_frames:
        print(f"Error: {file_name} interval ({start_frame}-{end_frame}) exceeds total frame count ({total_frames}).")
        return

    frame_indices = np.linspace(start_frame, end_frame, num=25, dtype=int)
    return cap, frame_indices, video_name, total_frames
...
def get_frame_for_not_region(video_path, output_dir,index):
    result = get_frame_for_not(video_path,index)

    if result is None:
        print(f"Skipped: {video_path} is not a valid ote video.")
        return

    cap, frame_indices, video_name, total_frames= result

    video_name=f"{video_name}_1"
    ...
    for frame_num in frame_indices:
        ...
        frame_filename = f"1_{frame_num}.jpg"
        frame_path = os.path.join(output_dir, frame_filename)
...
for video in os.listdir("./data"):
    ...
    get_frame_for_not_region(video_path,not_folder,index)
