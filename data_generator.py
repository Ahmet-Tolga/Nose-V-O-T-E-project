...
def parse_video_name(file_name,is_v):
    ...
    if is_v:
        try:
            ...
            print(f"Error with file name: {file_name}")
            return
...
def get_frame(video_path,is_v):
    ...
    frame_indices = np.linspace(start_frame, end_frame, num=16, dtype=int)
...
def get_frame_for_ote(video_path, output_dir):
    cap,frame_indices,video_name,total_frames,label=get_frame(video_path,False)
...
for video in os.listdir("../data"):
    ...
    #Cause just 101 102 and later videos includes OTE information.
    if(int(video.split("_")[0])>100):
